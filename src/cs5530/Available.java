...
public class Available 
{
	...
	public List<Integer> displayTHAvialable(Statement stmt, StringBuilder stringBuilder) throws IOException
	{
		String query = "Select t.THID, t.THname From THData t, Available a Where a.THID = t.THID";
		String output = "";
		List<Integer> THIDS = new ArrayList<Integer>();
		...
		if(THIDS.size() > 0)
		{
			stringBuilder.append("Here are the THID's of the temporary houses you that are avilable and the names of them: \n");
			stringBuilder.append(output);
		}
		else
		{
			stringBuilder.append("There are no temporary houses available \n");
		}
		return THIDS;
	}
	...
	public List<Integer> displayPidAvialable(int THID, Statement stmt, StringBuilder stringBuilder) throws IOException
	{
		String query = "Select p.pid, p.fromDate, p.toDate, a.pricePerNight From Available a, Period p Where a.THID = " + THID + " AND a.pid = p.pid";
		String output = "";
		ResultSet rs=null;
		...
		if(PIDS.size() > 0)
		{
			stringBuilder.append("Here are the pid's of the dates that the house you are looking at are available and the cost per night: \n");
			stringBuilder.append(output);
		}
		else
		{
			stringBuilder.append("There are no periods available for this house \n");
		}
		return PIDS;
	}
	
	public void displayReservations(int THID, int pid, Statement stmt, StringBuilder stringBuilder)
	{
